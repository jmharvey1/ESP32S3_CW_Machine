...
{
    TIMER_TRACE("begin");
     //sprintf(LogBuf,"lv_timer.c lv_timer_handler() Start\n");
    ...
    /*Run all timer from the list*/
     //sprintf(LogBuf,"lv_timer.c lv_timer_handler()  Run all timer from the list; Start\n");
    lv_timer_t * next;
    do {
        timer_deleted             = false;
        timer_created             = false;
            ...
            if(lv_timer_exec(LV_GC_ROOT(_lv_timer_act))) { //->lvgl/src/misc/lv_timer.c:314 (discriminator 2)
                ...
                if(timer_created || timer_deleted) {
                    TIMER_TRACE("Start from the first timer again because a timer was created or deleted");
                     //sprintf(LogBuf,"lv_timer.c lv_timer_handler() Start 1st timer again, timer%p was created or deleted\n", _lv_timer_act);
                    break;
                }
            }
            //}
     ...
     //sprintf(LogBuf,"lv_timer.c lv_timer_handler() Find Next Pending Delay; Start\n");
    ...
    while(next) {
     ...
     //sprintf(LogBuf,"lv_timer.c lv_timer_handler() Find Next Pending Delay; Complete\n");
...
static bool lv_timer_exec(lv_timer_t * timer)
{
    ...
    if(lv_timer_time_remaining(timer) == 0) {
        ...
        TIMER_TRACE("calling timer callback: %p", *((void **)&timer->timer_cb));
         //sprintf(LogBuf,"lv_timer.c lv_timer_exec()  calling timer%p callback: %p\n", timer, *((void **)&timer->timer_cb));
        //printf(LogBuf);
        if(timer->timer_cb && original_repeat_count != 0) timer->timer_cb(timer);
        TIMER_TRACE("timer callback %p finished", *((void **)&timer->timer_cb));
         //sprintf(LogBuf,"lv_timer.c lv_timer_exec()  timer callback %p finished\n", *((void **)&timer->timer_cb));
